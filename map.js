/* ==========================================================================
   Participant map — aggregates the live registration sheet by "Coming From"
   and plots it on a free OpenStreetMap/Leaflet map. No backend, no database,
   no stored lat/lng, no API key, no billing.

   HOW IT WORKS
   1. Fetch the same public Google Sheet CSV that registrations.js reads.
...
const SHEET = {
    // Same public 2026 registration sheet used by registrations.js
    sheetId: '1L03eFqUeOBekB6IBmLlvfYjfFyyzYw-2CtbvwrA01r0',
    gid: '308134922',
    columns: { name: 1, phone: 2, adults: 3, kids6to12: 4, kidsBelow6: 5, location: 6 }
};
...
async function init() {
    ...
    try {
        ...
        const { locations, familyCount } = aggregateByLocation(rows);
        renderDashboard(locations, familyCount);
        renderLocationTable(locations);

        if (locations.length === 0) {
            statusEl.textContent = 'No registrations with a "Coming From" location yet.';
            return;
        }
        ...
        console.error('Participant map failed to load:', error);
...
// spreadsheet footer/summary rows (a real issue seen on an older sheet in
// this project) by requiring a real name AND a real adult count. Returns
// both the per-location breakdown and the count of valid family rows.
function aggregateByLocation(rows) {
    ...
    for (const row of rows) {
        ...
        const adults = parseInt(row[c.adults], 10) || 0;
        const location = (row[c.location] || '').trim();
        ...
        entry.adults += adults;
        entry.kids6to12 += parseInt(row[c.kids6to12], 10) || 0;
        entry.kidsBelow6 += parseInt(row[c.kidsBelow6], 10) || 0;
...
// Dashboard tiles and the breakdown table are optional lookups — if either
// isn't present on the page, these just skip that part quietly.
...
/* ---------- 5. Map rendering (Leaflet + CARTO Positron) ---------- */

function initMap() {
    const map = L.map('bengaluruMap').setView(BENGALURU_CENTER, 11);
...
// match the site's green "paddy" accent (see .map-count-marker in styles.css).
...
async function plotLocations(map, locations) {
    const bounds = [];
    let placed = 0;
    let failed = [];
    ...
    for (const loc of locations) {
        ...
        if (coords) {
            const marker = L.marker([coords.lat, coords.lon], { icon: makeCountIcon(loc.total) }).addTo(map);
